...
- **v0.6.0** —— 空间 RBAC（四级角色矩阵 `Owner` / `Admin` / `Contributor` / `Viewer` + 按 KB 归属 + 每空间审计日志）、空间成员管理与多工作区 UX、自助创建工作区；`semiclaw` CLI v0.4 正式版 + `mcp serve`；KB 检索跨向量库扇出；MCP / 数据源凭据 AES-256-GCM 加密 + docreader gRPC TLS + Token；新增智谱 Embedding 与华为云 OBS；服务端用户偏好；Go 1.26.0。详见 [`docs/RBAC说明.md`](./docs/RBAC说明.md) 与 [`CHANGELOG.md`](./CHANGELOG.md)。
...
| MCP Server | 官方 PyPI 包 `tencent-semiclaw-mcp`，29 个工具，支持 stdio / SSE / HTTP 三种传输 |
...
| 界面 | Web UI / RESTful API / 命令行（`semiclaw`）/ Chrome Extension / 网站嵌入 Widget / 微信小程序 |
...
| 模型管理 | 集中配置，YAML 声明式内置模型配置，知识库级别模型选择，按模型思考模式与 Embedding 维度覆盖，交互式模型调试器，多空间共享内置模型，SemiClaw Cloud 托管模型与文档解析 |
...
[**`@wxg-prc-cpg/dsh-semiclaw`**](https://www.npmjs.com/package/@wxg-prc-cpg/dsh-semiclaw) 是官方的 [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness)（`dsh`）插件（[说明](./packages/dsh-semiclaw/README_CN.md)）。harness 自身不带任何检索、向量或知识库能力，这个插件把你的文档接进编码 Agent：`dsh plugin --profile web add @wxg-prc-cpg/dsh-semiclaw`，指向一个部署，Agent 的工具集里就会出现四个只读工具。

- **`semiclaw_search`** — 混合检索，返回原文片段，每条都带可复用的 `knowledge_id`
- **`semiclaw_read_document`** — 把单个文档的分块按序拼回正文，支持翻页
- **`semiclaw_ask`** — SemiClaw 自己带引用的成稿答案，走 RAG 或 ReAct 流水线
- **`semiclaw_list_knowledge_bases`** — 知识库名称与 id，便于 Agent 自己缩小检索范围
