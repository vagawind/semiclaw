...
const accessState: ProtectedFileAccessState = (() => {
  ...
  const scope = window as typeof window & {
    __semiclawProtectedFileAccessV1__?: ProtectedFileAccessState;
  };
  scope.__semiclawProtectedFileAccessV1__ ||= fresh();
  return scope.__semiclawProtectedFileAccessV1__;
})();
...
function tenantRequestHeaders(): Record<string, string> {
  ...
  try {
    const token = (localStorage.getItem('semiclaw_token') || '').trim();
    if (token) {
      headers['Authorization'] = `Bearer ${token}`;
    }

    const selectedTenantId = (localStorage.getItem('semiclaw_selected_tenant_id') || '').trim();
    if (selectedTenantId) {
      ...
      // active tenant into semiclaw_tenant, leaving authenticated file
