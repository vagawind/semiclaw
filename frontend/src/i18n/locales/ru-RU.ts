export default {
  ...
  integrations: {
    cli: {
      ...
      connectDesc: 'Создайте и активируйте профиль semiclaw, затем войдите с электронной почтой и паролем. Если такой профиль уже существует, выберите другое имя и измените имя профиля в примере MCP.',
      ...
      mcpDesc: 'После входа добавьте эту конфигурацию в клиент MCP с поддержкой stdio. Если клиент не находит semiclaw, укажите в command абсолютный путь к бинарному файлу.',
      copy: 'Копировать',
      copied: 'Скопировано',
    },
    ...
    claw: {
      title: 'SemiClaw Skill',
      subtitle: 'Импорт документов и гибридный поиск (вектор + ключевые слова) через REST API SemiClaw — загрузки, URL, Markdown и поиск.',
      ...
      ecosystemNote: 'Skill размещён на ClawHub ({\'@\'}lyingbug/semiclaw). Полная документация API — на странице ClawHub.',
      installCta: 'Открыть ClawHub',
      installCtaHint: 'Установка SemiClaw Skill · откроется в новой вкладке',
      hubMeta: 'ClawHub · {\'@\'}lyingbug/semiclaw · MIT-0',
      steps: {
        verify: {
          title: 'Проверка',
    ...
    chrome: {
      ...
      subtitle: 'Для self-hosted SemiClaw: вопросы в боковой панели, клиппинг страниц и Markdown-заметки в базы знаний прямо при просмотре.',
      ...
      steps: {
        ...
        port: {
          ...
          desc: 'В SemiClaw Desktop задайте фиксированный порт API (например 37841) в API-информации.'
        },
        api: {
    ...
    api: {
      ...
      capabilityManageModelsHint: 'Управление конфигурациями моделей, учётными данными, проверками подключения и данными SemiClawCloud.',
      ...
      hmacSecretDesc: 'Тот же secret, что в SemiClaw; только для подписи JWT на backend — не отправляйте его в заголовке запроса.',
      ...
      signedRequestStep0: '# 0. Подпишите JWT на backend (HS256, aud=semiclaw, sub=ID пользователя, tenant_id={tenantId}, exp не более 24 ч)',
  ...
  kbSettings: {
    parser: {
      ...
      engines: {
        ...
        semiclawcloud: {
          name: 'SemiClaw Cloud',
          desc: 'Парсинг документов через SemiClaw Cloud'
        },
        paddleocr_vl_cloud: {
  ...
  system: {
    ...
    versionDescription: 'Версия сервиса приложения (semiclaw-app)',
    frontendVersionLabel: 'Версия UI',
    frontendVersionDescription: 'Версия сборки UI (semiclaw-ui)',
  ...
  embedPublish: {
    ...
    originsHint: 'Укажите сайты, которым разрешено встраивание (A), а не адрес SemiClaw (B). Один полный Origin на строку, например https://shop.example.com; минимум один. Поддерживается *.example.com.',
    ...
    webhookUrlPlaceholder: 'https://your-server.example.com/semiclaw/embed-events',
    webhookUrlDesc: 'Optional. SemiClaw POSTs message_sent / message_received events to this HTTPS endpoint.',
    webhookSecret: 'Webhook secret',
    webhookSecretPlaceholder: 'HMAC-SHA256 secret (X-SemiClaw-Signature header)',
  ...
  createChat: {
    title: 'Привет, я SemiClaw — ваши знания всегда под рукой',
  ...
  auth: {
    ...
    firstTime: 'Впервые в SemiClaw?',
    registerSuccess: 'Регистрация завершена. Войдите в систему',
    registerFailed: 'Ошибка регистрации',
    subtitle: 'RAG, ReAct-агент и граф знаний — отраслевой экспертный мозг на основе больших моделей',
    registerSubtitle: 'Создайте аккаунт и начните работу с SemiClaw',
  ...
  envVarSettings: {
    ...
    description: 'Личные ключи для навыков и песочниц, а не системные или деплой-настройки SemiClaw.',
    ...
    nameInvalid: 'Это имя использовать нельзя. Имена, зарезервированные песочницей, например PATH или начинающиеся с SEMICLAW_, не принимаются.',
  ...
  settings: {
    ...
    sandbox: {
      ...
      backendDescriptions: {
        ...
        docker: 'Держит долгоживущий контейнер на каждую сессию на этом хосте SemiClaw; скрипты и файлы остаются в том же контейнере',
      },
      ...
      dockerHostRisk: 'Пустое значение или unix:// использует демон Docker на машине SemiClaw — это равносильно root на этой машине. Только для частной одноузловой установки. Если несколько пространств делят один хост, используйте Cube или E2B. Удалённый tcp:// требует каталог TLS-сертификатов.',
      ...
      semiclawTemplateTitle: 'SemiClaw standard template',
      semiclawDockerImage: 'SemiClaw standard image',
      semiclawDockerImageHint: 'Each session gets its own long-lived container. Scripts, shell commands and files all share it until the session ends or the idle timeout reclaims it.',
      semiclawTemplateOverview: 'SemiClaw provides the standard runtime. Templates are discovered after connecting and the standard one is created when missing.',
      semiclawTemplateDescription: 'Includes the Python, Node.js, CLI tools, workspace path, and non-root execution user expected by SemiClaw skills.',
      ...
      standardTemplateProvisioning: 'The SemiClaw standard template is being created. Refresh shortly to see its status.',
      ...
      templateUntaggedHint: 'Сборки завершены, но ни одна не имеет тега default, поэтому при создании песочницы шаблон не находится. Удалите его в E2B и обновите список — SemiClaw пересоберёт шаблон.',
      templateFailedReason: 'Сборка не удалась: {reason}',
      noTemplates: 'No templates were returned by this cluster.',
      semiclawStandardTemplate: 'Стандартный шаблон SemiClaw',
      ...
      semiclawDesktopTemplate: 'Шаблон рабочего стола SemiClaw',
      ...
      replaceStandardTemplateConfirm: 'Стандартный шаблон SemiClaw будет собран заново с текущими настройками, включая DNS. Предыдущий готовый шаблон не удаляется, пока замена не станет готовой к запуску.',
      desktopTemplateProvisioning: 'Шаблон рабочего стола SemiClaw создаётся. Обновите список чуть позже, чтобы увидеть статус.',
      ...
      replaceDesktopTemplateConfirm: 'Шаблон рабочего стола SemiClaw будет собран заново с текущими настройками, включая DNS. Предыдущий готовый шаблон рабочего стола не удаляется, пока замена не станет готовой к запуску. Шаблон CLI не затрагивается.',
      ...
      cubeDnsServersHelp: 'Необязательно. IP nameserver, которые записываются в стандартный шаблон SemiClaw. Пустое значение оставляет значение кластера по умолчанию (обычно 119.29.29.29). Если UDP/53 до публичных резолверов недоступен, укажите достижимые адреса из /etc/resolv.conf хоста Cube, исключая 10/8, 172.16/12 и 192.168/16. Чтобы изменение вступило в силу, на карточке шаблона нажмите «Пересобрать».',
      ...
      dockerTlsCertPathHelp: 'Directory on the SemiClaw host holding ca.pem, cert.pem and key.pem. Required for a remote daemon; certificates are mounted by the deployment, never stored here.',
      dockerIdleTtl: 'Idle reclaim (seconds)',
      dockerIdleTtlHelp: 'The Docker daemon has no idle timeout of its own. A container that runs no command for this long is reclaimed by SemiClaw and rebuilt when the session continues. Empty means 1800 seconds.',
    ...
    storage: {
      ...
      pathPrefixPlaceholder: 'напр. semiclaw/images',
      prefixPlaceholder: 'напр. semiclaw',
    ...
    semiclawCloud: {
      title: 'SemiClaw Cloud',
      description: 'Настройте учётные данные APPID и APPSECRET для SemiClaw Cloud. Данные используются для модельных сервисов и движка парсинга документов.',
      ...
      expired: 'Учётные данные SemiClaw Cloud истекли',
      ...
      appIdLabel: 'APPID',
      appIdDesc: 'ID приложения SemiClaw Cloud',
      appIdPlaceholder: 'Введите APPID',
      appSecretLabel: 'APPSECRET',
      appSecretDesc: 'Секрет приложения SemiClaw Cloud',
      ...
      addModelsDisplayName: {
        chat: 'SemiClawCloud Chat',
        embedding: 'SemiClawCloud Embedding',
        rerank: 'SemiClawCloud ReRank',
        vllm: 'SemiClawCloud Vision'
      },
      modelsSection: {
        title: 'Облачные модели',
        descReady: 'Зарегистрируйте четыре стандартные модели SemiClaw Cloud для чата, поиска, реранкинга и зрения.',
  ...
  newUserGuide: {
    ...
    steps: {
      ...
      knowledge: {
        ...
        desc: 'С этого всё начинается: загружайте документы, веб-страницы или FAQ, и SemiClaw автоматически разберёт и проиндексирует их. Нажмите здесь, чтобы открыть базы знаний.'
      },
      welcome: {
        title: 'Добро пожаловать в SemiClaw',
