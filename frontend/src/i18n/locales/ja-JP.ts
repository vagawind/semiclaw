export default {
  ...
  newUserGuide: {
    ...
    steps: {
      ...
      knowledge: {
        title: 'ナレッジベースを作成',
        desc: 'すべてはここから始まります。ドキュメント、Webページ、FAQをアップロードすると、SemiClawが自動で解析してインデックスを作成します。ここをクリックしてナレッジベースを開いてください。'
      },
      chat: {
        title: 'AIチャットを開始',
  ...
  settings: {
    ...
    sandbox: {
      ...
      backendDescriptions: {
        ...
        docker: 'このSemiClawホスト上でセッションごとに長時間稼働するコンテナを保持します。スクリプトとファイルはそのコンテナ内に残ります',
      },
      ...
      dockerHostRisk: '空欄またはunix://を指定するとSemiClawホストのDockerデーモンを使用します。これはそのマシンのroot権限に相当します。単一ノードのプライベート環境でのみ使用してください。複数のワークスペースがホストを共有する場合はCubeまたはE2Bを推奨します。リモートのtcp://エンドポイントにはTLS証明書ディレクトリが必要です。',
      ...
      semiclawTemplateTitle: 'SemiClaw標準テンプレート',
      semiclawDockerImage: 'SemiClaw標準イメージ',
      semiclawDockerImageHint: 'セッションごとに専用の長時間稼働コンテナが割り当てられます。スクリプト、シェルコマンド、ファイルはすべてこのコンテナを共有し、セッション終了またはアイドルタイムアウトによる回収まで保持されます。',
      semiclawTemplateOverview: 'SemiClawが標準ランタイムを提供します。テンプレートは接続後に検出され、存在しない場合は標準テンプレートが作成されます。',
      semiclawTemplateDescription: 'SemiClawのスキルが必要とするPython、Node.js、CLIツール、ワークスペースパス、非root実行ユーザが含まれます。',
      recommendedTag: '推奨',
      ...
      standardTemplateProvisioning: 'SemiClaw標準テンプレートを作成しています。しばらくしてから更新してステータスを確認してください。',
      ...
      templateUntaggedHint: 'ビルドは完了しましたが、デフォルトタグが付いたものがないため、サンドボックス作成時にこのテンプレートを解決できません。E2Bで削除して更新すると、SemiClawが再構築します。',
      templateFailedReason: 'ビルドに失敗しました: {reason}',
      noTemplates: 'このクラスタからテンプレートが返されませんでした。',
      semiclawStandardTemplate: 'SemiClaw標準テンプレート',
      createStandardTemplate: '作成',
      createStandardTemplateHint: 'DNSを含む現在の接続設定でビルドされます。設定を変更した場合は、カードから再構築してください。',
      semiclawDesktopTemplate: 'SemiClawデスクトップテンプレート',
      ...
      replaceStandardTemplateConfirm: 'DNSを含む現在の設定でSemiClaw標準テンプレートを再構築します。既存の起動可能なテンプレートは、新しいテンプレートの準備が整うまで削除されません。',
      desktopTemplateProvisioning: 'SemiClawデスクトップテンプレートを作成しています。しばらくしてから更新してステータスを確認してください。',
      desktopTemplateReplaced: '以前のデスクトップテンプレートを削除し、再構築を開始しました。完了までお待ちください。CLIテンプレートは変更しません。',
      desktopTemplateTag: 'デスクトップ',
      replaceDesktopTemplateConfirm: 'DNSを含む現在の設定でSemiClawデスクトップテンプレートを再構築します。既存の起動可能なデスクトップテンプレートは、新しいテンプレートの準備が整うまで削除されません。CLIテンプレートは変更しません。',
      ...
      cubeDnsServersHelp: '任意。SemiClaw標準テンプレートに書き込まれるネームサーバのIPです。空欄の場合はクラスタのデフォルト（通常は119.29.29.29）を使用します。パブリックリゾルバへのUDP/53がブロックされている場合は、Cubeホストの/etc/resolv.confから到達可能なアドレス（10/8、172.16/12、192.168/16を除く）を指定してください。既存の標準テンプレートには、テンプレートカードで「再構築」を実行した後にのみ反映されます。',
      ...
      dockerTlsCertPathHelp: 'SemiClawホスト上でca.pem、cert.pem、key.pemを格納しているディレクトリです。リモートデーモンでは必須です。証明書はデプロイ側でマウントされ、ここには保存されません。',
      dockerIdleTtl: 'アイドル回収（秒）',
      dockerIdleTtlHelp: 'Dockerデーモン自体にはアイドルタイムアウトがありません。この時間コマンドが実行されなかったコンテナはSemiClawが回収し、セッションが続く場合は再作成します。空欄の場合は1800秒です。',
    ...
    storage: {
      ...
      pathPrefixPlaceholder: '例: semiclaw/images',
      prefixPlaceholder: '例: semiclaw',
      bucketName: 'バケット名',
  ...
  envVarSettings: {
    ...
    description: 'スキルとサンドボックスで使う個人用のキーです。SemiClawのシステム設定やデプロイ設定ではありません。',
    ...
    nameInvalid: 'この名前は使用できません。PATHやSEMICLAW_で始まる名前など、サンドボックスが予約している名前は指定できません。',
  ...
  createChat: {
    title: 'こんにちは、SemiClawです。あなたのナレッジを、すぐそばに',
  ...
  embedPublish: {
    ...
    originsHint: 'SemiClaw の URL（B）ではなく、埋め込みを許可するホストサイト（A）を入力します。1 行に 1 つの Origin（例: https://shop.example.com）。1 件以上必須。*.example.com に対応。',
    ...
    webhookUrlDesc: '任意。SemiClawがmessage_sent / message_receivedイベントをこのHTTPSエンドポイントにPOSTします。',
    webhookSecret: 'Webhookシークレット',
    webhookSecretPlaceholder: 'HMAC-SHA256シークレット（X-SemiClaw-Signatureヘッダー）',
  ...
  system: {
    ...
    versionDescription: 'アプリケーションサービス（semiclaw-app）のバージョン',
    frontendVersionLabel: 'UIバージョン',
    frontendVersionDescription: 'UI（semiclaw-ui）のビルドバージョン',
  ...
  kbSettings: {
    ...
    parser: {
      ...
      engines: {
        ...
        semiclawcloud: {
          name: 'SemiClaw Cloud',
          desc: 'SemiClaw Cloudによるドキュメント解析'
        },
        markitdown: {
          name: 'MarkItDown',
  ...
  integrations: {
    cli: {
      ...
      connectDesc: 'semiclawという名前のプロファイルを作成して有効化し、メールアドレスとパスワードでサインインします。同名のプロファイルが既にある場合は別の名前を選び、MCPの例もそれに合わせて変更してください。',
      verifyTitle: '接続を確認',
      ...
      mcpDesc: 'サインイン後、この設定をstdio対応のMCPクライアントに追加してください。クライアントがsemiclawを見つけられない場合は、commandをバイナリの絶対パスに変更してください。',
      copy: 'コピー',
      copied: 'コピーしました',
    },
    title: '公開と連携',
    ...
    api: {
      ...
      capabilityManageModelsHint: 'モデル定義、認証情報、接続確認、SemiClawCloudの認証情報を管理します。',
      ...
      signedFlowDetail: 'HMACシークレットはSemiClawと信頼できる自社バックエンドにのみ保存し、リクエストに含めたりブラウザに配布したりしないでください。リクエストヘッダーで送るのは、そのシークレットで署名したJWT文字列（発行ごと、または有効期限切れ後に変わります）であり、シークレット自体ではありません。JWTにはsub（外部ユーザID）、tenant_id、aud=semiclaw、exp（有効期間は最大24時間）を含める必要があります。',
      ...
      hmacSecretDesc: 'SemiClawに保存されているものと同じシークレットです。JWTの署名のためバックエンドでのみ使用し、リクエストヘッダーとして送信しないでください。',
      ...
      signedRequestStep0: '# 0.バックエンドでJWTに署名（HS256、aud=semiclaw、sub=ユーザID、tenant_id={tenantId}、exp<=24h）',
    ...
    chrome: {
      title: 'ナレッジアシスタント',
      subtitle: 'セルフホスト版SemiClaw向け: ブラウジング中にサイドバーで質問したり、Webページをクリップしたり、Markdownメモをナレッジベースに保存したりできます。',
      ...
      steps: {
        ...
        port: {
          ...
          desc: 'SemiClawデスクトップ版では、API情報で固定ポート（例: 37841）を設定すると、再起動してもURLが変わりません。'
        },
        install: {
          title: '拡張機能をインストール',
    ...
    claw: {
      ...
      subtitle: 'SemiClaw REST API経由でドキュメントをインポートし、ハイブリッド検索（ベクトル＋キーワード）を実行します。ファイルのアップロード、URLインポート、Markdownでの登録、ナレッジベース横断検索に対応します。',
      ...
      steps: {
        ...
        env: {
          title: '環境変数を設定',
          desc: 'シェルまたは~/.zshrc / ~/.bashrcでSEMICLAW_BASE_URLとSEMICLAW_API_KEYを設定します。下記の例では現在のAPIベースURLを使用しています。APIキーは実際の値に置き換えてください。'
        },
        install: {
          title: 'スキルをインストール',
      ...
      ecosystemNote: 'スキルはClawHub（{\'@\'}lyingbug/semiclaw）でホストされています。完全なAPIドキュメントとバージョン履歴はClawHubページをご覧ください。',
      installCta: 'ClawHubを開く',
      installCtaHint: 'SemiClaw Skillをインストール · 新しいタブで開きます',
