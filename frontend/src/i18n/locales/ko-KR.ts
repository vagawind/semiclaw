export default {
  ...
  integrations: {
    cli: {
      ...
      connectDesc: 'semiclaw 프로필을 생성하고 활성화한 후 이메일과 비밀번호로 로그인하세요. 같은 이름의 프로필이 있다면 다른 이름을 사용하고 MCP 예제의 프로필 이름도 변경하세요.',
      verifyTitle: '연결 확인',
      ...
      mcpDesc: '로그인 후 stdio를 지원하는 MCP 클라이언트에 아래 설정을 추가하세요. 클라이언트가 semiclaw를 찾지 못하면 command를 바이너리의 절대 경로로 변경하세요.',
      copy: '복사',
      copied: '복사됨',
    },
    title: '게시 및 통합',
    ...
    claw: {
      ...
      subtitle: 'SemiClaw REST API로 문서를 가져오고 하이브리드 검색(벡터+키워드)을 수행합니다. 파일/URL/Markdown 업로드 및 검색에 사용.',
      ...
      ecosystemNote: 'Skill은 ClawHub({\'@\'}lyingbug/semiclaw)에 호스팅됩니다. 전체 API 문서는 ClawHub 페이지를 참고하세요.',
      installCta: 'ClawHub 열기',
      installCtaHint: 'SemiClaw Skill 설치 · 새 탭에서 열림',
    ...
    chrome: {
      title: '지식 관리 어시스턴트',
      subtitle: '자체 호스팅 SemiClaw와 함께: 사이드바에서 질문하고, 웹 페이지를 클리핑하며, Markdown 메모를 지식베이스에 저장하세요.',
      ...
      steps: {
        ...
        port: {
          title: '데스크톱: 고정 포트(권장)',
          desc: 'SemiClaw 데스크톱에서는 API 정보에서 고정 포트(예: 37841)를 설정하세요.'
        },
        api: {
          title: 'API 자격 증명',
    ...
    api: {
      ...
      capabilityManageModelsHint: '모델 설정, 자격 증명, 연결 테스트 및 SemiClawCloud 자격 증명을 관리합니다.',
      ...
      signedFlowDetail: 'HMAC secret은 SemiClaw와 신뢰할 수 있는 백엔드에만 보관하세요. 요청에 넣거나 브라우저에 배포하지 마세요. 요청 헤더에는 secret이 아니라 해당 secret으로 서명한 JWT 문자열을 넣습니다(발급마다 또는 만료 후 변경). JWT에는 sub(외부 사용자 ID), tenant_id, aud=semiclaw, exp(최대 24시간)가 필요합니다.',
      ...
      hmacSecretDesc: 'SemiClaw에 저장된 것과 동일한 secret. 백엔드에서 JWT 서명에만 사용하며 요청 헤더로 보내지 않습니다.',
      ...
      signedRequestStep0: '# 0. 백엔드에서 JWT 서명 (HS256, aud=semiclaw, sub=사용자 ID, tenant_id={tenantId}, exp 최대 24시간)',
  ...
  system: {
    ...
    retry: '재시도',
    versionLabel: '앱 버전',
    versionDescription: '애플리케이션 서비스(semiclaw-app)의 버전 번호',
    frontendVersionLabel: 'UI 버전',
    frontendVersionDescription: 'UI(semiclaw-ui) 빌드 버전 번호',
  ...
  embedPublish: {
    ...
    originsHint: 'SemiClaw 주소(B)가 아닌 임베드를 허용할 호스트 웹사이트(A)를 입력하세요. 한 줄에 하나의 Origin(예: https://shop.example.com), 최소 1개. *.example.com 지원.',
  ...
  createChat: {
    title: '안녕하세요, SemiClaw입니다 — 당신의 지식을 손끝에',
  ...
  auth: {
    ...
    firstTime: 'SemiClaw가 처음이신가요?',
    registerSuccess: '가입이 완료되었습니다. 로그인해주세요',
    registerFailed: '가입 실패',
    subtitle: "RAG Q&A, ReAct 에이전트, 지식 그래프 — 대규모 언어 모델 기반 산업급 전문가 브레인",
    registerSubtitle: '계정을 만들고 SemiClaw를 시작하세요',
  ...
  envVarSettings: {
    title: '샌드박스 키',
    description: '스킬과 샌드박스에 쓰는 개인 키이며, SemiClaw 시스템이나 배포 설정이 아닙니다.',
    helpAria: '샌드박스 키 설명',
    ...
    nameInvalid: '이 이름은 사용할 수 없습니다. PATH처럼 샌드박스가 예약한 이름이나 SEMICLAW_로 시작하는 이름은 허용되지 않습니다.',
  ...
  settings: {
    ...
    sandbox: {
      ...
      backendDescriptions: {
        ...
        docker: '이 SemiClaw 호스트의 Docker에서 세션마다 장수명 컨테이너를 유지합니다. 스크립트와 파일이 같은 컨테이너에 남습니다',
      },
      ...
      dockerHostRisk: '비우거나 unix:// 를 쓰면 SemiClaw가 있는 머신의 Docker 데몬을 사용하며, 그 권한은 해당 머신의 root와 같습니다. 프라이빗 단일 노드에만 쓰세요. 여러 워크스페이스가 같은 호스트를 쓰는 경우에는 Cube 또는 E2B를 쓰세요. 원격 tcp:// 는 TLS 인증서 디렉터리가 필요합니다.',
      addConfig: '샌드박스 추가',
      ...
      templateUntaggedHint: '빌드는 끝났지만 default 태그가 붙은 빌드가 없어 샌드박스 생성 시 이 템플릿을 찾을 수 없습니다. E2B에서 템플릿을 삭제하고 새로고침하면 SemiClaw가 다시 빌드합니다.',
      templateFailedReason: '빌드 실패: {reason}',
      noTemplates: 'No templates were returned by this cluster.',
      semiclawStandardTemplate: 'SemiClaw 표준 템플릿',
      createStandardTemplate: '만들기',
      createStandardTemplateHint: '현재 연결 설정(DNS 포함)으로 구축합니다. 설정을 바꾼 뒤에는 카드에서 재구축하세요.',
      semiclawDesktopTemplate: 'SemiClaw 데스크톱 템플릿',
      ...
      replaceStandardTemplateConfirm: '현재 설정(DNS 포함)으로 SemiClaw 표준 템플릿을 다시 만듭니다. 새 템플릿이 준비되기 전에는 기존에 쓸 수 있는 템플릿을 삭제하지 않습니다.',
      desktopTemplateProvisioning: 'SemiClaw 데스크톱 템플릿을 만들고 있습니다. 잠시 후 새로고침해 상태를 확인하세요.',
      desktopTemplateReplaced: '기존 데스크톱 템플릿을 삭제했고 재구축을 시작했습니다. 준비될 때까지 기다리세요. CLI 템플릿은 그대로 둡니다.',
      desktopTemplateTag: '데스크톱',
      replaceDesktopTemplateConfirm: '현재 설정(DNS 포함)으로 SemiClaw 데스크톱 템플릿을 다시 만듭니다. 새 템플릿이 준비되기 전에는 기존에 쓸 수 있는 데스크톱 템플릿을 삭제하지 않으며, CLI 템플릿은 그대로 둡니다.',
      ...
      cubeDnsServersHelp: '선택 사항입니다. SemiClaw 표준 템플릿에 기록되는 nameserver IP입니다. 비워 두면 클러스터 기본값(보통 119.29.29.29)을 사용합니다. 공용 DNS로 UDP/53이 나가지 않으면 Cube 호스트 /etc/resolv.conf에서 도달 가능한 주소를 넣고, 10/8, 172.16/12, 192.168/16은 피하세요. 이미 있는 표준 템플릿은 카드에서 「재구축」을 눌러야 변경이 적용됩니다.',
    ...
    storage: {
      ...
      pathPrefixPlaceholder: '예: semiclaw/images',
      prefixPlaceholder: '예: semiclaw',
    ...
    semiclawCloud: {
      ...
      addModelsDisplayName: {
        chat: 'SemiClawCloud 대화',
        embedding: 'SemiClawCloud Embedding',
        rerank: 'SemiClawCloud ReRank',
        vllm: 'SemiClawCloud 비전'
      },
      modelsSection: {
        title: '클라우드 모델',
        descReady: 'SemiClaw Cloud의 네 가지 표준 모델을 등록하여 대화, 검색, 재정렬, 비전에 사용합니다.',
  ...
  newUserGuide: {
    ...
    steps: {
      ...
      knowledge: {
        title: '지식 베이스 만들기',
        desc: '모든 것의 시작점입니다. 문서, 웹페이지, FAQ를 업로드하면 SemiClaw가 자동으로 분석하고 색인합니다. 여기를 클릭해 지식 베이스로 이동하세요.'
      },
      welcome: {
        title: 'SemiClaw에 오신 것을 환영합니다',
