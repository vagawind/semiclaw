...
export const EMBED_CHAT_SESSION_STORAGE_PREFIX = 'semiclaw-embed-session:'

/** localStorage key prefix for anonymous embed visitor ids (per channel). */
export const EMBED_VISITOR_STORAGE_PREFIX = 'semiclaw-embed-visitor:'
...
const EMBED_MSG_SOURCE = 'semiclaw-embed'
const EMBED_HOST_SOURCE = 'semiclaw-host'
...
export function buildWidgetSnippet(
  ...
  const attrs = [
    `src="${escapeHtmlAttr(`${base}/semiclaw-widget.js`)}"`,
...
export const SECURE_TOKEN_ENDPOINT_PLACEHOLDER = 'https://your-backend.example.com/semiclaw/embed-token'
...
export function buildSecureServerNodeExample(channelId: string, opts?: { baseUrl?: string }) {
  ...
  return [
    `// Node/Express — keep SEMICLAW_PUBLISH_TOKEN only on the server (env var).`,
...
export function buildSecureServerGoExample(channelId: string, opts?: { baseUrl?: string }) {
  ...
  const exchangeUrl = `${base}/api/v1/embed/${channelId}/exchange`
  return [
    `// Go net/http — keep SEMICLAW_PUBLISH_TOKEN only on the server (env var).`,
