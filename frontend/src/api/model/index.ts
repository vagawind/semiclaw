...
export interface InitializeSemiClawCloudRequest {
  app_id: string
  app_secret: string
}

// 仅保存 SemiClawCloud 凭证，不自动创建模型
export function saveSemiClawCloudCredentials(data: InitializeSemiClawCloudRequest): Promise<{ success: boolean; message: string }> {
  return new Promise((resolve, reject) => {
    post('/api/v1/semiclawcloud/credentials', data)
      .then((response: any) => {
        if (response.success) {
          resolve(response)
        } else {
      ...
      .catch((error: any) => {
        console.error('Failed to save SemiClawCloud credentials:', error)
        reject(error)
      })
  })
}

export interface SemiClawCloudStatusResult {
  has_models: boolean
  needs_reinit: boolean
  reason?: string
}

export function getSemiClawCloudStatus(): Promise<SemiClawCloudStatusResult> {
  return new Promise((resolve, reject) => {
