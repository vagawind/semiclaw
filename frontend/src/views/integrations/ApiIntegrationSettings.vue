...
type SemiClawDesktopWindow = Window & {
...
// Send on each SemiClaw API request:
...
async function tryLoadWailsApiBaseURL() {
  const win = window as SemiClawDesktopWindow
  for (let i = 0; i < 40; i++) {
...
async function tryLoadWailsLanHints(win: SemiClawDesktopWindow) {
...
function desktopPortBindingsAvailable(win: SemiClawDesktopWindow) {
...
function desktopBindPublicBindingsAvailable(win: SemiClawDesktopWindow) {
  const app = win.go?.main?.App
  return (
...
const onDesktopBindPublicChange = async (value: boolean) => {
  ...
  const fn = (window as SemiClawDesktopWindow).go?.main?.App?.SetDesktopHTTPBindPublicSetting
...
const saveDesktopPort = async () => {
  ...
  const fn = (window as SemiClawDesktopWindow).go?.main?.App?.SetDesktopHTTPPortSetting
  if (typeof fn !== 'function') return
  try {
...
function openApiDoc() {
  window.open('https://github.com/vagawind/semiclaw/blob/main/docs/api/README.md', '_blank')
}
