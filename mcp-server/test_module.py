...
def test_imports():
    ...
    try:
        ...
        print("✓ semiclaw_mcp_server 模块导入成功")

        # 测试包导入
        from semiclaw_mcp_server import SemiClawClient, run

        print("✓ SemiClawClient 和 run 函数导入成功")
...
def test_environment():
    ...
    print(f"SEMICLAW_BASE_URL: {base_url or '未设置 (将使用默认值)'}")
    print(f"SEMICLAW_API_KEY: {'已设置' if api_key else '未设置'}")

    if not base_url:
        print("提示: 可以设置环境变量 SEMICLAW_BASE_URL")

    if not api_key:
        print("提示: 建议设置环境变量 SEMICLAW_API_KEY")
...
def test_wiki_tools():
    ...
    try:
        ...
        for method in ["wiki_search", "wiki_read_page", "wiki_index_view"]:
            assert hasattr(client, method), f"SemiClawClient 缺少方法: {method}"
            assert callable(getattr(client, method)), f"{method} 不可调用"
            print(f"✓ SemiClawClient.{method} 存在")
...
def main():
    """运行所有测试"""
    print("SemiClaw MCP Server 模组测试")
    print("=" * 50)
