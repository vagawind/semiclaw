#!/usr/bin/env python3
"""
SemiClaw MCP Server 主入口点

这个文件提供了一个统一的入口点来启动 SemiClaw MCP 服务器。
可以通过多种方式运行：
1. python main.py
2. python -m semiclaw_mcp_server
3. semiclaw-mcp-server (安装后)
...
def check_environment_variables():
    ...
    print("=== SemiClaw MCP Server 环境检查 ===", file=sys.stderr)
    ...
    if not base_url:
        print("提示: 可以设置 SEMICLAW_BASE_URL 环境变量", file=sys.stderr)

    if not api_key:
        print("警告: 建议设置 SEMICLAW_API_KEY 环境变量", file=sys.stderr)
...
环境变量:
  SEMICLAW_BASE_URL       SemiClaw API 基础 URL (默认: http://localhost:8080/api/v1)
  SEMICLAW_API_KEY        SemiClaw API 密钥
  MCP_SERVER_AUTH_TOKEN  SSE/HTTP 传输必填，客户端通过 Authorization: Bearer 传递
    ...
    parser.add_argument(
        "--version", action="version", version="SemiClaw MCP Server 1.1.1"
    )
...
async def main():
    ...
    try:
        print(f"正在启动 SemiClaw MCP Server (transport={args.transport})...", file=sys.stderr)

        from semiclaw_mcp_server import run_stdio, run_sse, run_http
