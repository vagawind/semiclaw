...
SemiClaw 的数据源导入模块支持从外部平台（飞书、企业微信、Notion、Confluence 等）自动导入和同步内容到知识库。用户可配置数据源连接，选择需要同步的资源，并通过手动触发或定时调度自动完成内容的增量/全量同步。
...
从飞书知识库（Wiki）同步文档到 SemiClaw。支持 docx、doc、sheet、bitable、file 类型文档的自动导入。
...
开启“同步删除”后，确认源端删除的文档会从 SemiClaw 知识库移除；关闭该开关则保留本地内容。
...
每个数据源代表一个外部平台连接与 SemiClaw 知识库的绑定关系。一个知识库可以添加多个数据源，实现从多个平台汇聚内容。
...
飞书连接器支持从飞书知识库（Wiki）同步文档到 SemiClaw。
