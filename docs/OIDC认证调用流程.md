# SemiClaw OIDC 认证调用流程

本文档说明 SemiClaw 当前 OIDC 登录能力的实际调用过程，覆盖：
...
和常见的纯前端 OIDC SDK 不同，SemiClaw 的特点是：
...
3. 后端拿到 OIDC 用户信息后，会：
   ...
   - 最终签发 SemiClaw 自己的本地 JWT（`token` / `refresh_token`）。
...
因此，**OIDC Provider 的 token 只用于后端换取用户身份，本项目真正的业务访问凭证仍然是 SemiClaw 自己签发的 JWT**。
...
浏览器随后离开 SemiClaw 页面，跳转到 OIDC Provider 的授权页。
...
第四个参数是首次自动开户时的默认空间策略，由 `resolveDefaultTenantMode` 从 `auth.default_tenant_mode`（SystemSetting，DB > `SEMICLAW_AUTH_DEFAULT_TENANT_MODE` > 配置文件）解析，与本地密码注册共用同一开关；OIDC 不再有独立的 `OIDC_AUTH_DEFAULT_TENANT_MODE`。
...
因此，**首次使用 OIDC 登录的用户，不需要提前在 SemiClaw 中手工建号**。
...
> OIDC 只负责“确认你是谁”，SemiClaw 自己负责“签发系统内可用的业务令牌”。
...
5. **真正用于访问 SemiClaw API 的仍是本地 JWT**，不是 OIDC access token。
