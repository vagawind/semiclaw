...
SemiClaw 的数据源导入模块支持从外部平台（飞书、钉钉、企业微信、Notion、Confluence 等）自动导入和同步内容到知识库。用户可配置数据源连接，选择需要同步的资源，并通过手动触发或定时调度自动完成内容的增量/全量同步。
...
```
外部平台 API → Connector → ConnectorRegistry → DataSourceService → SemiClaw Core (知识入库管道)
```
