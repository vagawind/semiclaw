...
启动方式：`docker compose --profile doris up -d`，然后在 FE 上 `CREATE DATABASE semiclaw;`。
...
`TENCENT_VECTORDB_COLLECTION` 是集合名前缀。SemiClaw 会按向量维度创建实际集合，例如 `semiclaw_embeddings_768`。
