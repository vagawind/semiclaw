...
本文提供了向 SemiClaw 项目添加新向量数据库支持的完整指南。通过实现标准化接口和遵循结构化流程，开发者可以高效地集成自定义向量数据库。
...
通过遵循以上步骤和参考现有实现，你可以成功集成新的向量数据库到 SemiClaw 系统中，扩展其向量检索能力。
...
SemiClaw 通过 `database/sql + go-sql-driver/mysql` 调用 MySQL 协议；
通过 `net/http` 调用 Stream Load。两条通道复用同一份用户名/密码。

### 表结构与维度分表

每个 embedding 维度对应一张物理表 `<DORIS_TABLE_PREFIX>_<dim>`（如 `semiclaw_embeddings_768`）。
表关键属性：
...
注：Doris ANN 索引在建表后**异步构建**，索引未就绪期间查询会退化为 brute-force（结果正确，速度较慢）。SemiClaw 在 `ensureTable` 中会轮询 `SHOW INDEX FROM <table>` 等待 `idx_emb` 进入 `FINISHED`/`NORMAL` 状态，超时上限 30s。
...
docker exec -it SemiClaw-doris-fe mysql -h 127.0.0.1 -P 9030 -uroot \
    -e "CREATE DATABASE IF NOT EXISTS semiclaw;"
```

随后启动 SemiClaw 后端，知识库写入即会按维度自动建表。

## Tencent VectorDB

SemiClaw 内置 Tencent VectorDB 适配器，驱动名为 `tencent_vectordb`。该适配器支持向量检索、基于 BM25 sparse vector 的关键词检索和索引管理，可参与 SemiClaw 上层混合检索。
...
`TENCENT_VECTORDB_COLLECTION` 是集合名前缀。SemiClaw 会按向量维度创建实际集合，例如 `semiclaw_embeddings_768`，用于隔离不同 embedding 模型维度的数据。
