...
- Check availability and install missing dependencies supported by this sandbox within the installer
  scope. Put standalone CLI binaries in <skill-dir>/.semiclaw/bin (on PATH when a session selects this
...
- With write_skill_file, create .semiclaw/install-report.json as a JSON object with two required fields:
  ...
  credentials; declare ordinary per-user API keys in .semiclaw/requirements.json instead of blockers.
...
func (s *TenantSkillService) verifyRuntimePrerequisites(
	...
	raw, err := reader.ReadSessionFile(ctx, sessionID, path.Join(skillDir, ".semiclaw", "install-report.json"))
	if err != nil {
		return fail(
			"Write .semiclaw/install-report.json after assessing CLI and external runtime prerequisites: "+err.Error(),
			true,
		)
	}
	...
	if err := json.Unmarshal(raw, &report); err != nil || report.Commands == nil || report.Blockers == nil {
		return fail(`Write a valid .semiclaw/install-report.json with commands and blockers arrays of strings`, true)
	}
