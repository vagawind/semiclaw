...
func NewSemiClawCloudEmbedder(config Config) (*SemiClawCloudEmbedder, error) {
	...
	if baseURL == "" {
		baseURL = provider.SemiClawCloudBaseURL
	}
...
func (e *SemiClawCloudEmbedder) BatchEmbed(ctx context.Context, texts []string) ([][]float32, error) {
	...
	for _, item := range embedResp.Data {
		if item.Index < 0 || item.Index >= len(result) {
			return nil, fmt.Errorf(
				"semiclawcloud embedder: response index %d out of range for %d inputs",
				item.Index,
				len(texts),
			)
		}
		if seen[item.Index] {
			return nil, fmt.Errorf("semiclawcloud embedder: duplicate response index %d", item.Index)
		}
	...
	for index, found := range seen {
		if !found {
			return nil, fmt.Errorf("semiclawcloud embedder: missing embedding for input index %d", index)
		}
	}
	return result, nil
}
