...
// `semiclaw schema` command can introspect any command's contract without
// invoking its help func (and toggling SEMICLAW_AGENT_HELP). Shares the
// annotations map with SetRisk (distinct keys).
const agentHelpAnnotation = "semiclaw.agent_help"
...
//
// The AgentHelp is also recorded as a JSON annotation so `semiclaw schema` can
