...
release. Grab the latest from the [Releases page](https://github.com/vagawind/semiclaw/releases).
...
For AI agents (Claude Code, Cursor, Gemini CLI, etc.) integrating SemiClaw:

1. Install: `brew install semiclaw` or `go install github.com/vagawind/semiclaw/cli@latest`
...
```bash
ln -s "$PWD/skills/semiclaw-shared"     ~/.claude/skills/semiclaw-shared
ln -s "$PWD/skills/semiclaw-rag-search" ~/.claude/skills/semiclaw-rag-search
```
...
flag, or MCP tool the CLI no longer has. (A `semiclaw skills install` command is
...
```bash
semiclaw kb list --format json                              # [{ "id": "kb_x", "name": "Eng" }, …]
semiclaw kb view kb_x --format json                         # { "id": "kb_x", "name": "Eng", … }
semiclaw kb list --format json --jq '.[] | {id, name}'      # project to listed fields
semiclaw kb list --format json --jq '.[].id'                # jq over the bare data
```
...
The `semiclaw session continue-stream` command resumes an SSE event stream for an existing assistant message. Useful for network-blip recovery or polling long-running agent invocations:
...
semiclaw session continue-stream sess_abc --message msg_xyz
