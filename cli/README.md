# semiclaw — SemiClaw CLI

A command-line interface for the SemiClaw RAG knowledge-base server. Lets you
...
```bash
$ semiclaw --help
Command-line client for the SemiClaw RAG server. Manage knowledge bases
...
Available Commands:
  ...
  api         Make a raw API request to the SemiClaw server
  ...
  mcp         Run semiclaw as a Model Context Protocol server
...
cd SemiClaw/cli
go build -o semiclaw .
sudo mv semiclaw /usr/local/bin/   # or anywhere on $PATH
```
...
```bash
# 1. Register your SemiClaw server as a profile and make it active
...
echo "sk-..." | semiclaw auth login --with-token

# 3. List knowledge bases
semiclaw kb list

# 4. Bind this directory to a knowledge base — subsequent commands auto-resolve --kb
semiclaw link --kb my-knowledge-base

# 5. Upload a document, then block until parsing finishes
semiclaw doc upload notes.md
semiclaw doc wait doc_abc                          # exit 0 completed, 1 failed, 124 --timeout, 130 ^C
semiclaw doc reparse doc_abc                       # re-trigger parsing if it failed, then wait again

# 6. Search
semiclaw search chunks "what is reciprocal rank fusion?"

# 7. Ask the LLM (streams to terminal)
semiclaw chat "summarise the design doc"

# 8. Manage custom agents and run them (see `semiclaw agent --help` / `semiclaw session --help`)
semiclaw model list                               # discover a model id for --model
semiclaw agent list
semiclaw session ask --agent ag_abc "what's our q4 retention plan?"

# 9. Inspect a document's chunks for RAG retrieval debug
semiclaw chunk list --doc doc_xyz

# 10. Inspect messages in a session / search across sessions
semiclaw message list --session sess_abc
semiclaw message search "retry policy"                      # cross-session Q&A retrieval

# 11. Resolve a pending tool approval (agent run blocked on approval event)
semiclaw session tool-approval resolve pend_xxx -y          # approve (after user go-ahead)
semiclaw session resume sess_abc --message msg_xyz # resume the blocked stream

# 12. Health & verification verbs
semiclaw kb status kb_abc       # fast snapshot: reachable / counts / processing flag (1 HTTP)
semiclaw kb check kb_abc        # deep verify: also aggregates failed_count via doc list (1+N HTTP)
semiclaw agent status ag_abc    # fast: reachable / model_id
semiclaw agent check ag_abc     # deep: probes every KB in the agent's scope
```
...
For AI agents (any MCP-capable host) integrating SemiClaw:
...
2. Authenticate. In a sandbox / CI, the **stateless** path needs no `auth login`
   ...
   ```bash
   export SEMICLAW_API_KEY="sk-…"   # or SEMICLAW_TOKEN for a bearer JWT
   export SEMICLAW_HOST="https://kb.example.com"
   semiclaw kb list                 # already authenticated
   ```
   Or, for a persisted local profile:
   ```bash
   semiclaw profile add prod --host <server-url> --use
   semiclaw auth login
   ```
3. Register MCP in the host's MCP config:
   ```json
   {"mcpServers": {"semiclaw": {"command": "semiclaw", "args": ["mcp", "serve"]}}}
   ```
...
under [`skills/`](skills/) that teach an agent to drive SemiClaw without trial and error:

- [`semiclaw-shared`](skills/semiclaw-shared/SKILL.md) — **read first**: auth/profile
  sequence, `--kb` resolution, the JSON-envelope + exit-code contract, the exit-10
  protocol, `--dry-run`, and CLI-vs-MCP selection.
- [`semiclaw-rag-search`](skills/semiclaw-rag-search/SKILL.md) — when to use `chat`
...
```bash
semiclaw skills install                      # writes to ~/.claude/skills
semiclaw skills install --dir <agent-skills-dir> --force   # other agents / overwrite
semiclaw skills list --format json           # what would be installed
```
...
```bash
semiclaw profile add prod    --host https://prod.example.com --use     # add + switch
semiclaw auth login                                                    # authenticate active (prod)

semiclaw profile add staging --host https://staging.example.com        # add (stays inactive)
echo "sk-..." | semiclaw --profile staging auth login --with-token     # authenticate staging

semiclaw auth list
semiclaw profile use prod                                              # switch back
```
...
under `$XDG_CONFIG_HOME/semiclaw/secrets/`. The active profile lives in
`~/.config/semiclaw/config.yaml`.

To remove a profile's stored credentials:

```bash
semiclaw auth logout                       # active profile
...
`semiclaw --version` bump. This section is the human overview; the complete,
...
```bash
semiclaw kb list --format json                              # {"ok":true,"data":[{"id":"kb_x",…}],"meta":{"count":1,…}}
semiclaw kb view kb_x --format json                         # {"ok":true,"data":{"id":"kb_x","name":"Eng",…}}
semiclaw kb list --format json --jq '.data[] | {id, name}'  # project listed fields out of each item
semiclaw kb list --format json --jq '.data[].id'            # ids only
semiclaw kb list --format json --jq '.meta.count'           # number returned
```
...
regardless of TTY — running `semiclaw kb list | jq` works without an
...
| `2`   | flag / argument validation error                       | re-check `semiclaw <cmd> --help` |
...
Run `semiclaw exit-codes` for the machine-readable matrix (JSON); `semiclaw help exit-codes` for the human-readable table.
...
- `semiclaw mcp serve` exposes a curated read-only tool surface over
  stdio MCP for any MCP-compatible client.
- `semiclaw schema` enumerates every command with its `used_for`, and
  `semiclaw schema <cmd path>` (e.g. `semiclaw schema doc update`) prints that
...
SemiClaw CLI exposes top use cases as polished commands; deep
...
- **Per-request `chat` parameters** — multi-KB scope, summary model
  override, image attachments, web search toggle. Use `semiclaw api POST
  /api/v1/knowledge-chat/<session-id> --input -`.
- **Per-request `session ask --agent` overrides** — same shape via
  `semiclaw api POST /api/v1/agent-chat/<session-id> --input -`.
- **Operations without a CLI verb** — register / change-password /
  OIDC flows, organization / sharing endpoints, tenant management.

`semiclaw api --help` documents the raw passthrough. Run
`semiclaw doctor` first to verify auth and base URL.
...
semiclaw kb create --name "test-kb" --description "for review" --dry-run
...
```bash
semiclaw kb delete kb_xxxx --dry-run   # exit 0, no prompt
semiclaw kb delete kb_xxxx             # exit 10, prompts for -y
```

For the `api` command, dry-run requires explicit write method (POST/PUT/PATCH/DELETE); GET returns FlagError:

```bash
echo '{"name":"foo"}' | semiclaw api -X POST /api/v1/knowledge-bases --input - --dry-run   # OK
semiclaw api /api/v1/knowledge-bases --dry-run                                              # exit 2: requires explicit -X
```
...
The `semiclaw session resume` command resumes an SSE event stream for an existing assistant message. Useful for network-blip recovery or polling long-running agent invocations:

```bash
# Original streaming call captures session_id + message_id from init event:
semiclaw session ask "..." --agent ag_xxxx --format ndjson | tee /tmp/stream.ndjson
# {"type":"init","session_id":"sess_abc","message_id":"msg_xyz"}
# ... events flow ...
# [network blip]

# Resume the same stream:
semiclaw session resume sess_abc --message msg_xyz
...
semiclaw session tool-approval resolve pend_xxx -y                      # approve
# semiclaw session tool-approval resolve pend_xxx --reject --reason "..." -y  # reject
...
Run `semiclaw doctor` for a 4-status diagnostic (OK / warn / fail /
...
| Verb | Cost | Use |
|---|---|---|
| `semiclaw kb status <kb-id>`     | 1 HTTP    | live counts / processing flag |
| `semiclaw kb check <kb-id>`      | 1+N HTTP  | adds `failed_count` via doc-list page-walk |
| `semiclaw agent status <agent-id>` | 1 HTTP  | reachable / model_id |
| `semiclaw agent check <agent-id>`  | 1+N HTTP | also probes every KB in the agent's scope |

`semiclaw doc wait <doc-id> [<doc-id>...]` blocks until each document
...
# Run the real-server e2e suite (requires SEMICLAW_E2E_HOST + token env vars)
...
- **Bugs and feature requests**: file an issue at
  [github.com/vagawind/semiclaw/issues](https://github.com/vagawind/semiclaw/issues).
