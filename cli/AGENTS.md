...
```json
{
  ...
  "error": {
    ...
    "retry_argv": ["semiclaw", "auth", "login"],
...
| | |
|---|---|
| **SemiClaw** | two separate fields: `retry_argv` (suggested next argv as a string array, directly executable for non-destructive errors; informational only on exit-10) + `hint` (prose) |
...
The `semiclaw session resume <session-id> --message <msg-id>` command resumes an SSE event stream for an existing assistant message. Use cases: network-blip recovery, long-running agent invocation polling, completed-stream inspection.
...
After TTL, `semiclaw session resume` returns the typed error `local.sse_stream_aborted`, which maps to exit code 1 per the Error code reference.
