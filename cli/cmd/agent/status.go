...
For downstream KB reachability verification use 'semiclaw agent check <id>'
(active verification, 1 + N HTTP). For full agent config / metadata use
'semiclaw agent view <id>'.`,
		Example: `  semiclaw agent status ag_abc
  semiclaw agent status ag_abc --format json`,
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples:      []string{"semiclaw agent status agent_abc"},
