// Package agentcmd holds the `semiclaw agent` command tree:
...
//
// "agent" in this subtree refers to SemiClaw's user-defined Custom
// Agents (server resource: GET/POST /agents/...) and handles CRUD
// operations only. Agent invocation has moved to `semiclaw session ask
...
// NewCmd builds the `semiclaw agent` parent and registers leaves. Called
...
or delete agents. To invoke an agent, use: semiclaw session ask --agent <id>`,
	}
