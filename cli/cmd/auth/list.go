...
// NewCmdList builds `semiclaw auth list`. Per-host enumeration: render one
// row per registered profile, marking the active one. Reads only
// ~/.config/semiclaw/config.yaml - no network, no keyring touch.
func NewCmdList(f *cmdutil.Factory) *cobra.Command {
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples: []string{"semiclaw auth list", "semiclaw auth list --jq '.data[].name'"},
...
func runList(fopts *cmdutil.FormatOptions, f *cmdutil.Factory) error {
	...
	if len(entries) == 0 {
		fmt.Fprintln(iostreams.IO.Out, "No profiles configured. Run `semiclaw auth login` to create one.")
		return nil
	}
