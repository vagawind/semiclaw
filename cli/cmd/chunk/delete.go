...
func NewCmdDelete(f *cmdutil.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			batchRetry := append([]string{"semiclaw", "chunk", "delete"}, args...)
...
func runDelete(ctx context.Context, opts *DeleteOptions, fopts *cmdutil.FormatOptions, svc DeleteService, p prompt.Prompter) error {
	if err := cmdutil.ConfirmDestructive(p, opts.Yes, fopts.WantsJSON(), "delete", "chunk", opts.ChunkID, "chunk.delete", []string{"semiclaw", "chunk", "delete", opts.ChunkID, "--doc", opts.DocID, "-y"}); err != nil {
		return err
	}
