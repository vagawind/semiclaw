...
// NewCmdDelete builds `semiclaw model delete <model-id>`.
...
agent still referencing the model will fail until repointed.`,
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			if err := cmdutil.ConfirmDestructive(f.Prompter(), opts.Yes, fopts.WantsJSON(),
				"delete", "model", id, "model.delete", []string{"semiclaw", "model", "delete", id, "-y"}); err != nil {
				return err
			}
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples:      []string{"semiclaw model delete model_abc -y"},
