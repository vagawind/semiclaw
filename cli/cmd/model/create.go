...
// NewCmdCreate builds `semiclaw model create <name>`.
...
summary config (see 'semiclaw kb config set') or an agent (--model).
...
Two modes:
  ...
  Remote (API):    --source remote --provider <name> [--api-key-stdin] [--base-url <url>]
                   ...
                   live provider catalog (semiclaw api /api/v1/models/providers);
...
provider key via --api-key-stdin so it never lands in argv/history. Anything
	...
	cmd.Flags().StringVar(&opts.Provider, "provider", "", "Remote provider id, required+validated with --source remote (see `semiclaw api /api/v1/models/providers`)")
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		UsedFor:       "register a model (embedding/rerank/chat/VLLM/ASR) so a KB or agent can use it; capture .data.id to pass to `semiclaw kb config set` / `agent create --model`.",
		RequiredFlags: []string{"<name> (positional)", "--type", "--source (local|remote)", "--provider (when --source remote)"},
		Examples: []string{
			`semiclaw model create nomic-embed-text --type Embedding --source local --dimension 768   # Ollama (server pulls it)`,
			`printf '%s' "$OPENAI_KEY" | semiclaw model create text-embedding-3-small --type Embedding --source remote --provider openai --dimension 1536 --api-key-stdin`,
		},
