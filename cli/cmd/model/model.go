// Package modelcmd holds the `semiclaw model` command tree: list / view.
//
// These are read-only discovery commands. The model id is a required input to
// `semiclaw agent create --model <id>` and to a knowledge base's embedding /
// summary model settings, so the CLI must offer a first-class way to find it
// rather than forcing callers down to the raw `semiclaw api` escape hatch.
//
...
// NewCmd builds the `semiclaw model` parent and registers leaves. Called from
// cli/cmd/root.go.
...
the model id to back a knowledge base's embedding / summary config ('semiclaw kb
init') or an agent ('semiclaw agent create --model <id>').`,
	}
