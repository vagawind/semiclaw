...
Server-side ingestion knobs:
  ...
  semiclaw doc upload ./docs --recursive --glob '*.pdf' --metadata team=alpha`,
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		RequiredFlags: []string{"<file> (positional)", "--kb (or SEMICLAW_KB_ID / project link)"},
		Examples: []string{
			`semiclaw doc upload report.pdf --kb kb_eng`,
			`semiclaw doc upload report.pdf --kb kb_eng --jq .data.id   # then: doc wait <id>`,
		},
