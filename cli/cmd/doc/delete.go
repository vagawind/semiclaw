...
without the user's explicit go-ahead.`,
  ...
  semiclaw doc delete --all --kb=kb_x -y --format json # agent-friendly`,
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			if opts.All {
				if opts.KB == "" {
					return cmdutil.NewError(cmdutil.CodeInputInvalidArgument, "--all requires --kb=<id>").
						...
						WithRetryArgv([]string{"semiclaw", "doc", "delete", "--all", "--kb=<kb-id>", "-y"})
				}
				if len(args) > 0 {
			...
			batchRetry := append([]string{"semiclaw", "doc", "delete"}, args...)
...
func runDelete(ctx context.Context, opts *DeleteOptions, fopts *cmdutil.FormatOptions, svc DeleteService, p prompt.Prompter, id string) error {
	if err := cmdutil.ConfirmDestructive(p, opts.Yes, fopts.WantsJSON(), "delete", "document", id, "doc.delete", []string{"semiclaw", "doc", "delete", id, "-y"}); err != nil {
		return err
	}
...
func runDeleteAll(ctx context.Context, opts *DeleteOptions, fopts *cmdutil.FormatOptions, svc AllService, p prompt.Prompter) error {
	if err := cmdutil.ConfirmDestructive(p, opts.Yes, fopts.WantsJSON(), "delete", "all docs in KB", opts.KB, "doc.delete_all", []string{"semiclaw", "doc", "delete", "--all", "--kb=" + opts.KB, "-y"}); err != nil {
		return err
	}
