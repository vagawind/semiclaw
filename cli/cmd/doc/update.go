...
// NewCmdUpdate builds `semiclaw doc update <doc-id>`. At least one of
...
to the user first.`,
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			retryCmd := cmdutil.BuildRetryArgv(c, []string{"semiclaw", "doc", "update", id}, "title", "description", "format")
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples: []string{
			`semiclaw doc update doc_abc --title "Q3 Runbook" -y`,
			`semiclaw doc update doc_abc --description "archived" --format json -y`,
		},
