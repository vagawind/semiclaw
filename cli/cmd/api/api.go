...
Examples:
  ...
  semiclaw api /api/v1/knowledge-bases -d '{"name":"foo"}'              # POST (auto)
  semiclaw api /api/v1/knowledge-bases -F name=foo -F enabled=true      # POST, typed body
  echo '{"name":"foo"}' | semiclaw api /api/v1/knowledge-bases --input -  # POST via stdin
  semiclaw api /api/v1/knowledge-bases/kb_xxx -X DELETE`,
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			// as `semiclaw kb delete` - exit-10 destructive protocol must apply
			...
			case http.MethodDelete:
				if err := cmdutil.ConfirmDestructive(f.Prompter(), opts.Yes, fopts.WantsJSON(), "delete", "endpoint", args[0], "api.delete", []string{"semiclaw", "api", "-X", "DELETE", args[0], "-y"}); err != nil {
					return err
				}
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples: []string{
			...
			"semiclaw api /api/v1/knowledge-bases -d '{\"name\":\"foo\"}'",
			"semiclaw api -X POST /api/v1/knowledge-bases -F name=\"Eng Docs\" -F enabled=true",
			"semiclaw api -X DELETE /api/v1/knowledge-bases/kb_x -y",
			"echo '{\"name\":\"foo\"}' | semiclaw api /api/v1/knowledge-bases --input -",
		},
...
// apiRetryArgv reconstructs a directly-executable `semiclaw api` argv (with -y)
...
func apiRetryArgv(opts *Options, method, path string) []string {
	argv := []string{"semiclaw", "api", "-X", method, path}
	switch {
	case opts.Data != "":
