...
func emptyContentSearchHint(n int) string {
	...
	return "0 results: this may be no match, OR the KB has no indexed chunks. " +
		"Check `semiclaw kb status <kb>` (chunk_count) and `semiclaw doc list --kb <kb>` (parse_status); " +
		"documents in parse_status=draft are not indexed — run `semiclaw doc reparse <doc-id>`."
}
