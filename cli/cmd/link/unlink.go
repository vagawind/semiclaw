...
// NewCmdUnlink builds `semiclaw unlink`. Walks up from cwd so a user in
...
func NewCmdUnlink() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Removes the .semiclaw/project.yaml that ` + "`semiclaw link`" + ` previously
...
is present anywhere in the parent chain.`,
		Example: `  semiclaw unlink           # remove the binding for this project
  semiclaw unlink --format json    # bare JSON (CI / agents)`,
		Args: cobra.NoArgs,
		RunE: func(c *cobra.Command, _ []string) error {
			...
			if !found {
				return &cmdutil.Error{
					...
					Hint:    "run `semiclaw link --kb <id>` first, or check you're in the right directory",
				}
			}
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		UsedFor: "Remove the .semiclaw/project.yaml KB binding from the current directory tree. No flags required; walks up from cwd to find the link.",
