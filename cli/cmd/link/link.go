...
Pass the knowledge base as a positional argument (semiclaw link <id-or-name>) or
...
user explicitly asked to bind this directory; don't run it as a side effect.`,
		Example: `  semiclaw link a32a63ff-fb36-4874-bcaa-30f48570a694         # positional UUID
  semiclaw link engineering                                  # positional name → id
  semiclaw link --kb engineering                             # --kb form (equivalent)
  semiclaw link                                              # interactive (TTY)`,
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		UsedFor:       "Bind the current directory to a knowledge base by writing .semiclaw/project.yaml. Give the KB as a positional arg or --kb (non-interactive); only run when the user explicitly asks to link this directory.",
		RequiredFlags: []string{"<kb> positional or --kb (required when no TTY)"},
		Examples:      []string{"semiclaw link engineering", "semiclaw link --kb a32a63ff-fb36-4874-bcaa-30f48570a694"},
...
func resolveProfile(f *cmdutil.Factory) (string, error) {
	...
	if cfg.CurrentProfile == "" {
		...
		// profile — env credentials (SEMICLAW_API_KEY) alone have no profile to
		...
		return "", cmdutil.NewError(cmdutil.CodeAuthUnauthenticated,
			...
			WithHint("register one with `semiclaw profile add <name> --host <url> --use`; for a headless (SEMICLAW_API_KEY) workflow, skip `link` and pass --kb per command or set SEMICLAW_KB_ID").
