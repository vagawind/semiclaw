...
// NewCmdStatus builds `semiclaw kb status <id>`.
...
For deep verification including failed_count, use 'semiclaw kb check <id>'
(1 + N HTTP, pages the doc list with parse_status=failed).

For full metadata (config / pinned / tenant), use 'semiclaw kb view <id>'.`,
		Example: `  semiclaw kb status kb_abc
  semiclaw kb status kb_abc --format json`,
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples:      []string{"semiclaw kb status kb_abc"},
...
func retrievalHint(ready bool) string {
	...
	return "  ← no embedding model bound; run `semiclaw kb config set <id>`"
}
