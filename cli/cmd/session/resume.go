// resume.go implements `semiclaw session resume` —
...
// NewCmdResume builds `semiclaw session resume <session-id> --message <id>`.
...
silently treated as NDJSON.`,
		Example: `  semiclaw session resume sess_xyz --message msg_abc
  semiclaw session resume sess_xyz -m msg_abc --format ndjson`,
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		RequiredFlags: []string{"<session-id> (positional)", "--message (persisted assistant message id — get it from `semiclaw message list --session <id>`; a live stream's assistant_message_id is not resumable once the message persists)"},
		Examples: []string{
			"semiclaw session resume sess_xyz --message msg_abc --format json",
			"# Get the message id from: semiclaw message list --session <session-id> (the persisted assistant message)",
		},
