...
without the user's explicit go-ahead.`,
  ...
  semiclaw session delete s_a s_b s_c -y         # delete 3, keep-going`,
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			batchRetry := append([]string{"semiclaw", "session", "delete"}, args...)
...
func runDelete(ctx context.Context, opts *DeleteOptions, fopts *cmdutil.FormatOptions, svc DeleteService, p prompt.Prompter, id string) error {
	if err := cmdutil.ConfirmDestructive(p, opts.Yes, fopts.WantsJSON(), "delete", "session", id, "session.delete", []string{"semiclaw", "session", "delete", id, "-y"}); err != nil {
		return err
	}
