// Package skillscmd holds the `semiclaw skills` command tree: list the bundled
...
// NewCmd builds the `semiclaw skills` parent command.
...
func newCmdList() *cobra.Command {
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		UsedFor:  "list the Agent Skills bundled in this CLI binary (name, description, file count) — discover what `semiclaw skills install` would write",
		Examples: []string{"semiclaw skills list --format json"},
...
Existing files are left untouched unless --force is passed.`,
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples: []string{"semiclaw skills install --dry-run --format json", "semiclaw skills install --dir ~/.claude/skills --force"},
