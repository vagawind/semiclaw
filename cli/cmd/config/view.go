...
// NewCmdView builds `semiclaw config view`. Read-only inspection of the
...
"(unresolved)").`,
	...
	cmdutil.SetAgentHelp(cmd, cmdutil.AgentHelp{
		...
		Examples: []string{
			"semiclaw config view",
			"semiclaw config view --format json",
			"semiclaw config view --jq '.data.active_profile'",
		},
...
// resolveProjectLink reports the discovered .semiclaw/project.yaml path or
