# Changelog — `semiclaw` CLI

All notable changes to the `semiclaw` CLI (the binary under `cli/` in this
...
of the SemiClaw server / frontend release cadence.
...
#### Added
- `semiclaw session stop <session-id>` command to abort an in-flight agent run.
...
- **`auth login` drops `--host` and `--name`.** It authenticates the active profile; create it first with `semiclaw profile add <name> --host <h> --use`. Target a non-active profile with the global `semiclaw --profile <name> auth login`.
...
#### Added
- `--dry-run` flag on every mutation cobra command (`kb create/edit/delete`, `agent create/edit/delete`, `doc create/upload/fetch/delete`, `chunk delete`, `session delete`, `auth refresh/logout`, `link/unlink`, `profile add/remove`) and on `semiclaw api` (POST/PUT/PATCH/DELETE only; GET returns FlagError exit 2).
- envelope `meta.dry_run: true` + `meta.plan: {action, args | method+path+body}` open-map fields (omitempty in non-dry-run envelopes).
- `semiclaw session continue-stream <session-id> --message <msg-id>` command for SSE event stream replay/recovery.
...
#### Changed
- `SetAgentHelp` wrapper in `cli/internal/cmdutil/agenthelp.go` now prepends "Risk:" line in default (non-JSON) help branch when `cmd.Annotations["risk.action"]` is set. SEMICLAW_AGENT_HELP=1 JSON path unchanged.
...
- **`--format` default flips to `json` regardless of TTY.**
  ...
  - Migration: humans on a TTY pass `--format text` (or set
    `SEMICLAW_FORMAT=text` env) for the prior auto-text behavior.
...
- **`semiclaw agent invoke` removed; use `semiclaw session ask --agent <id>`.**
  - Server route is `POST /sessions/{session_id}/agent-qa` — session-anchored.
  - `semiclaw agent` keeps CRUD only (list / view / create / edit / delete /
    status / check).
  - Migration: `semiclaw agent invoke ag_x "Q"` →
    `semiclaw session ask --agent ag_x "Q"` (auto-creates session if none given).
- **`semiclaw doc upload` split into three commands.**
  - `semiclaw doc upload <file>` — local file only.
  - `semiclaw doc fetch <url>` — server-side remote fetch (was `upload --from-url`).
  - `semiclaw doc create --text "..."` — direct text knowledge.
...
- **`semiclaw kb empty` removed; use `semiclaw doc delete --all --kb=<id>`.**
  - Atomic server `ClearKnowledgeBaseContents` (no list-then-delete race).
  - Same exit-10 `-y/--yes` guard as `kb delete`.
  - Migration: `semiclaw kb empty kb_x -y` →
    `semiclaw doc delete --all --kb=kb_x -y`.
- **`semiclaw api -d/--data` flag removed; use `--input <file>` or `--input -`
  (stdin).**
  - `semiclaw api` now accepts any non-empty HTTP method (whitelist removed)
    so the escape hatch can hit endpoints the CLI doesn't natively model.
  - Migration: `semiclaw api -d '{"foo":1}' /endpoint` →
    `echo '{"foo":1}' | semiclaw api --input - /endpoint`.
- **Batch operations envelope shape — per-item `ok` pattern.**
  - `semiclaw doc delete id1,id2,id3` and similar multi-id mutations now emit:
...
- **Unknown subcommand emits typed envelope.**
  - `input.unknown_subcommand` with `detail.{unknown, command_path, available[]}`
    ...
    `"unknown command \"x\" for \"semiclaw\""` prose.
- **`semiclaw chat` requires the query as a single quoted argument.**
  - v0.6: `MinimumNArgs(1)` silently joined `semiclaw chat hello world` into
    `"hello world"`.
  - v0.7: `ExactArgs(1)` rejects multi-arg with exit 2; matches
    `semiclaw session ask`. Quote the query: `semiclaw chat "hello world"`.

#### Added
- **`SEMICLAW_PROFILE` env var** selects the active profile for a single
...
- **`SEMICLAW_FORMAT` env var** sets the default `--format`. Values:
...
- **`error.retry_after_seconds`** — `server.rate_limited` / `server.timeout`
  surface server `Retry-After` header verbatim. CLI-direct (`semiclaw api`)
...
- **`semiclaw doc fetch <url>`** — new command (see split above).
- **`semiclaw doc create --text "..."`** — new command (see split above).
- **`semiclaw session ask --agent <id> "..."`** — new command (replaces
  `agent invoke`).
- **`semiclaw doc delete --all --kb=<id>`** — new mode of `doc delete`
  (replaces `kb empty`).
...
- **`SEMICLAW_SDK_DEBUG=1` env removed** → use **`SEMICLAW_LOG_LEVEL=debug`**.
...
- **`semiclaw doc wait <id> [<id>...]`** — block until every document reaches a
...
- **`--log-level error|warn|info|debug`** persistent flag + `SEMICLAW_LOG_LEVEL`
...
- **`semiclaw kb status <id>`** — fast health snapshot (1 HTTP). Returns
  reachable / counts / is_processing.
- **`semiclaw kb check <id>`** — deep verification: status fields + `failed_count`
  ...
  cost to the caller.
- **`semiclaw agent status <id>`** — fast health snapshot (1 HTTP):
  reachable / model_id.
- **`semiclaw agent check <id>`** — deep verification: status fields +
  `kb_scope_all_reachable` from probing each KB in scope (1 + N HTTP). Same
  status/check verb split as kb status/check.
- **`semiclaw doc delete <doc-id> [<doc-id>...]`** — positional multi-id.
  Default keep-going on failure. Single `-y/--yes` confirms the entire
  batch; non-TTY without `-y` still exits 10.
- **`semiclaw session delete <session-id> [<session-id>...]`** — positional
  multi-id with the same keep-going semantics as `doc delete`.
- **`semiclaw chunk delete <chunk-id> [<chunk-id>...] --doc <doc-id>`** — positional
  multi-id, all chunks share the same `--doc` parent (server route requires it).
- **`semiclaw api <path> --paginate`** — follows semiclaw's offset-based
...
- **`SetAgentHelp` pattern** — `cmdutil.SetAgentHelp(cmd, AgentHelp{...})`
  ...
  shape, activated by `SEMICLAW_AGENT_HELP=1` at `--help` time. Applied
...
#### Added
- `semiclaw agent create <name> --model <id>` / `agent edit <id>` /
...
- `semiclaw chunk list --doc <doc-id>` / `chunk view <chunk-id>` /
  `chunk delete <chunk-id> --doc <doc-id>` — new subtree for RAG retrieval
  debug. Paginated with v0.4 `--limit` / `--page-size` / `--all-pages` canon.
- `semiclaw mcp serve` adds `chunk_list` as the 10th curated tool.
- `semiclaw agent view <id>` human output now renders all 34 AgentConfig
...
- `semiclaw doc list` gains `--keyword` / `--file-type` / `--source` /
...
- `semiclaw session view --full` (with `--limit`, default 50, bounds
...
- `semiclaw kb view` human render now includes `TYPE`, `PINNED` (badge,
...
- `semiclaw doc view` human render expands to include `TITLE` (when
...
- `semiclaw doc upload` gains `--enable-multimodel` (tri-state:
...
#### Breaking changes
- `semiclaw search docs` now applies the keyword filter server-side via
  ...
  **The match is now case-sensitive** (server uses `LIKE %keyword%`),
  ...
  back to `semiclaw api` with a custom filter.
...
- `cli/README.md` now documents the `semiclaw api` raw HTTP passthrough
...
#### Added
- `semiclaw mcp serve` — curated read-only stdio MCP server exposing 9
...
- `semiclaw agent list` / `agent view` / `agent invoke` — manage and
  call SemiClaw's server-side Custom Agent resources.
- `semiclaw auth token` — print the active credential to `stdout` for
  scripting (raw secret by default; `--json` emits `{token, mode, context}`).
- `semiclaw doc upload --from-url` — ingest a remote URL.
...
- `auth login --with-token` validates the supplied key against
  ...
  `$XDG_CONFIG_HOME/semiclaw/secrets/`.
...
- `unlink` — remove the cwd's `.semiclaw/project.yaml` so subsequent
