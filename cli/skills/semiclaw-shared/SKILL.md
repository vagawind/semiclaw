---
name: semiclaw-shared
description: Use when driving a SemiClaw RAG server through the `semiclaw` CLI as an agent — authenticating, managing knowledge bases / documents / sessions / agents, running search or chat, or interpreting the CLI's JSON envelopes and exit codes. Read this before any other semiclaw-* skill.
metadata:
  tested_against: v0.10
---

# SemiClaw CLI — shared base

`semiclaw` is the agent-first CLI for a SemiClaw RAG server. Every command prints
a JSON envelope and uses a typed exit code, so you branch on machine-readable
output, not prose. Read this skill before any task-specific `semiclaw-*` skill.

## 1. Authenticate (do this first — order matters)

**Agents usually skip profiles entirely:** set `SEMICLAW_API_KEY` (or
`SEMICLAW_TOKEN`) + `SEMICLAW_HOST` and every command authenticates statelessly and
...
Authentication is a **two-step sequence**. `semiclaw auth login` operates on the
...
`semiclaw model list`) or bind after the fact
...
- `--jq '<expr>'` filters the envelope (e.g. `semiclaw kb list --jq '.data[].id'`).
- Exception: `semiclaw auth token` emits the **raw token** by default (it's a
...
| 2 | flag / argument validation (bad/unknown/missing-required flag) | re-check `semiclaw <cmd> --help` |
...
{"ok":false,"error":{"type":"input.confirmation_required",
  ...
  "retry_argv":["semiclaw","kb","delete","X","-y"],"risk":{"level":"destructive","action":"kb.delete"}}}
```
...
```bash
KB=$(semiclaw kb create "Docs" --jq '.data.id' --format json | tr -d '"')
semiclaw doc upload ./manual.pdf --kb "$KB"
```

## 7a. Reliability patterns for long-running agent runs

### Inspecting prior messages
Use `semiclaw message list --session <sess-id>` to review the message history of a session (e.g., after a stream drops) before deciding whether to re-ask or continue. Use `semiclaw message search "<query>"` to locate a prior Q&A exchange across all sessions — prefer this over re-running an expensive query when the answer may already exist.
...
3. After explicit user go-ahead: `semiclaw session tool-approval resolve <pending-id> -y` to approve, or add `--reject --reason "..."` to reject.
4. Resume the answer: `semiclaw session resume <sess-id> --message <msg-id>`.
...
link/unlink  bind cwd to a KB        mcp serve  expose semiclaw as MCP tools
```

**chat vs session ask vs search** — the most common confusion: see the
`semiclaw-rag-search` skill for the decision table. Briefly: `chat` = one-shot
...
For an IDE/host agent that speaks MCP, `semiclaw mcp serve` exposes a curated
...
Run `semiclaw <command> --help`. With `SEMICLAW_AGENT_HELP=1` set, `--help` emits a
...
| Mistake | Fix |
|---|---|
| `semiclaw auth login` before any profile exists | `profile add <n> --host <url> --use` first (§1) |
...
| `search chunks "q"` → exit 1 `local.kb_id_required` | Pass `--kb <name-or-id>`, set `SEMICLAW_KB_ID`, or `semiclaw link` the dir |
