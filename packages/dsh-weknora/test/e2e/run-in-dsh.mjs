 ...
 * against a fake OpenAI-compatible model and a mock SemiClaw backend, and assert
 ...
 *      visible set itself — SemiClaw refuses an unscoped search.
 *
 * Usage:
...
const QUESTION = 'SemiClaw 的默认检索阈值是多少？请查知识库后回答。'
const transcript = []
  ...
  return [
    {
      ...
      config: { provider: 'semiclaw-e2e', model: 'semiclaw-e2e-model' },
    },
    {
      id: 'llm-pi-ai',
      config: {
        providers: {
          'semiclaw-e2e': {
            displayName: 'SemiClaw e2e gateway',
            apiKeyEnv: 'SEMICLAW_E2E_MODEL_KEY',
            api: 'openai-completions',
            baseURL: modelUrl,
            models: [{
              id: 'semiclaw-e2e-model',
              name: 'SemiClaw e2e model',
              contextWindow: 65536,
              maxTokens: 4096,
            }],
          },
 ...
 * the tools, the tools reached SemiClaw, the retrieved passages reached the
 * model, and the printed answer is grounded in them.
 */
async function scenario(options) {
  const { label, dsh, home, env, patch, prefix, semiclaw, autoScope = false } = options
  const model = await startFakeModel({ searchTool: `${prefix}_search`, readTool: `${prefix}_read_document` })
  const requestsBefore = semiclaw.requests.length
  try {
    await writeFile(
    ...
    assert.match(dump.stdout, /name: ["']?@wxg-prc-cpg\/dsh-semiclaw/, 'the composed tree must mount the plugin')
    ...
    const calls = semiclaw.requests.slice(requestsBefore)
    const paths = calls.map(request => request.path)
    log(`# SemiClaw received: ${paths.join(', ')}`)
    assert.ok(paths.includes('/api/v1/knowledge-search'), 'search must reach the retrieval endpoint')
    if (autoScope) {
      // SemiClaw rejects an unscoped retrieval, so with nothing configured the
    ...
    assert.equal(
      semiclaw.requests.slice(requestsBefore).every(request => request.headers['x-api-key'] === API_KEY),
      true,
    ...
    log(`# PASS (${label}): ${prefix}_search and ${prefix}_read_document ran inside dsh against SemiClaw, `
...
async function main() {
  ...
  const semiclaw = await startMockWeknora({ apiKey: API_KEY })
  const home = await mkdtemp(join(tmpdir(), 'dsh-semiclaw-home-'))
  const dsh = await resolveDsh()
  const env = {
    DSH_HOME: home,
    SEMICLAW_E2E_MODEL_KEY: 'not-a-real-key',
    SEMICLAW_BASE_URL: semiclaw.url,
    SEMICLAW_API_KEY: API_KEY,
    SEMICLAW_KNOWLEDGE_BASE_IDS: 'kb-product',
  }

  log(`# mock SemiClaw   ${semiclaw.url}`)
  log(`# DSH_HOME       ${home}`)
  ...
  try {
    ...
    assert.ok(
      manifest.dsh.profile.bundles.includes('@wxg-prc-cpg/dsh-semiclaw'),
      'the bundle must join the profile layer list',
    )

    // A. The shipped bundle patch, configured only by environment variables.
    await scenario({ label: 'A · shipped bundle defaults from env', dsh, home, env, semiclaw, patch: [], prefix: 'semiclaw' })
    ...
    //    SEMICLAW_KNOWLEDGE_BASE_IDS leaves behind.
    await scenario({
      ...
      patch: [{
        id: 'semiclaw',
        config: { baseUrl: semiclaw.url, apiKey: API_KEY },
      }],
    })

    log('\n# ALL SCENARIOS PASSED')
  } finally {
    await semiclaw.close()
